...
Insertion: In the worst case, all keys hash to the same slot, making the time complexity O(n), where n is the number of keys. However, if the hash function is well-designed and distributes keys uniformly, the lengths of the chains remain small, and the average insertion time remains close to O(1).
Search/Retrieve: Similarly, the worst-case time complexity is O(n) when all keys collide into the same slot. The average case remains close to O(1) if collisions are minimal and well-managed.
Delete: Deleting an element also requires searching for it first in its chain, 
